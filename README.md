
# Hi there, I'm M Meenakshi! 👋

A passionate developer. I love building amazing projects with code. Currently exploring web development and more!

## 🚀 About Me
- 🔭 I’m currently working on: Open to new opportunities
- 🌱 I’m currently learning: React.js, Java
- 👯 I’m looking to collaborate on: Fun open-source projects
- 🤔 I’m looking for help with: Anything specific
- 💬 Ask me about: Web development, CSS, JavaScript


## 🛠️ Tech Stack
| Category    | Technologies                     |
|-------------|--------------------------------  |
| Frontend    | HTML, CSS, JavaScript, React.js  |
| Backend     | Java, Node.js                    |
| Databases   | MySql, MangoDB                   |
| Languages   | Python, JavaScript               |
| Tools       | Git                              |

## 📊 GitHub Stats

- Public Repos: 6
- Followers: 0

![Your GitHub Stats](https://github-readme-stats.vercel.app/api?username=Meena-21&show_icons=true&theme=radical)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Meena-21&layout=compact&theme=radical)

## 🔥 Pinned Repositories
| Repo                  | Description | Language     | Stars |
|-----------------------|-------------|--------------|-------|
| [MyPortfolio](https://github.com/Meena-21/MyPortfolio) |  This is my personal portfolio website created to showcase my web development skills, projects, and                                                                   technical experience. The portfolio highlights my ability to build responsive and interactive user                                                                   interfaces using modern web technologies.                                  | CSS          | ⭐ 0   |
| [TaskBuddy](https://task-buddy-bice.vercel.app/)       |  Task Buddy is a task management application built to help users organize their daily activities and improve productivity. It allows users to create, update, and manage tasks through a clean and user-friendly interface.                   | JavaScript   | ⭐ 0   |
| [Birdy-Match](https://birdy-match.vercel.app/)         | The main objective of the project is to improve user memory and concentration through an interactive                                                                 game interface.”“I developed the project using React for the frontend, JavaScript for logic, CSS for                                                                 styling, and GitHub for version control and deployment.                     | JavaScript   | ⭐ 0   |
| [git-tuts](https://github.com/Meena-21/git-tuts)       |                                                                               | Python       | ⭐ 0   |
| [react-practice](https://github.com/Meena-21/react-practice) |                                                                         | HTML         | ⭐ 0   |
| [PortfolioExample](https://github.com/Meena-21/PortfolioExample) |                                                                     | HTML         | ⭐ 0   |
| [Example](https://github.com/Meena-21/Example)         |                                                                               | HTML         | ⭐ 0   |



## 📫 Connect with Me
- 📧 Email: meenakshim7738@gmail.com
- 💼 LinkedIn: https://www.linkedin.com/in/m-meenakshi-032337280

---

⭐️ **From [Meena-21](https://github.com/Meena-21)**

